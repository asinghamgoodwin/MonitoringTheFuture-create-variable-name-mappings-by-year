...
Create an empty tibble to fill in with information that'll allow us to extract information on any question from any year. For this, we need a mapping from the `helpful_name` (something descriptive, like "age") to each year that question was asked, which file it's data was stored in, and what the variable name was.
...
*2-what form(s) a question was asked on*
...
Next, I want to clean up this database by making sure that everything that is the same question and *should* share a `helpful_name` actually does, because that's the way I'll associate the same question across different years. Continuing from above, I:
3. Arrange and group by `helpful_name`, and add a `count` column to see how many times that specfic name comes up across different years. 
4. Individually check and edit names that should have gone together, but didn't
```{r}
